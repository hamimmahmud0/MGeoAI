  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-30 flex w-full max-w-[560px] flex-col border-l border-line bg-surface shadow-xl" aria-label="Incident detail" aria-live="polite">
      ...
      <div className="flex-1 space-y-7 overflow-y-auto px-6 py-5">
        ...
        <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Location and precision</h3><div className="flex gap-3 rounded border border-line p-3"><MapPin size={18} className="mt-0.5 shrink-0 text-primary" /><div><p className="text-sm font-medium">{incident.geolocation.display_name || 'Unmapped'}</p><p className="mt-1 text-xs leading-5 text-muted">{incident.geolocation.granularity} precision · {Math.round(incident.geolocation.confidence * 100)}% confidence{incident.geolocation.uncertainty_radius_km ? ` · ~${incident.geolocation.uncertainty_radius_km} km uncertainty radius` : ''}</p>{incident.geolocation.ambiguity_reason && <p className="mt-2 text-xs text-muted">{incident.geolocation.ambiguity_reason}</p>}{incident.geolocation.latitude !== undefined && incident.geolocation.longitude !== undefined && incident.geolocation.method !== 'collection_country_fallback' && <a className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline" href={`https://www.openstreetmap.org/?mlat=${incident.geolocation.latitude}&mlon=${incident.geolocation.longitude}#map=15/${incident.geolocation.latitude}/${incident.geolocation.longitude}`} target="_blank" rel="noreferrer"><ExternalLink size={13} />Inspect nearby roads</a>}</div></div></section>
